...
>
> **Примечание для Pet_Ozon:** в этом проекте манифесты и скрипт деплоя лежат в `infra/rancher/`
> (а не в `rancher/` в корне, как в обобщённых примерах ниже). Деплой: `.\infra\rancher\deploy.ps1`.
> Пути вида `rancher/k8s/` ниже читай как `infra/rancher/k8s/`.
