...
### Что РЕАЛЬНО создаёт связку (доказано вживую на UAT, 2026-05-31)

**Связка создаётся ⇔ клиент поставил галочку «Сохранить карту» на форме банка.**
Проверено сквозным тестом (Playwright + curl) на `daniil77_test-api`:

| Действие | Результат |
|----------|-----------|
| Оплата с галочкой «Сохранить карту» (1-стадийный `register.do`+`clientId`) | `bindingInfo.bindingId` ✅, `getBindings` отдаёт связку |
| Оплата с галочкой (2-стадийный `registerPreAuth`) | `bindingInfo.bindingId` приходит уже при авторизации (orderStatus=1) ✅ |
| Оплата БЕЗ галочки | `bindingInfo` пуст → только `cardAuthInfo` → связки нет |
| `register*` + `features=AUTO_PAYMENT` | **errorCode 13**: «Мерчант не имеет привилегии выполнять AUTO платежи» |
| `register*` + `features=FORCE_CREATE_BINDING` | **errorCode 14**: «Отсутствует разрешение на принудительное создание связок» |

```
register.do / registerPreAuth.do (clientId) → formUrl
→ клиент СТАВИТ галочку «Сохранить карту» + платит   ← без неё связки НЕ будет
→ getOrderStatusExtended → bindingInfo.bindingId      ← реальный, списываемый
→ getBindings(clientId) → возвращает связку
```

> ⚠️ **Дважды опровергнутая легенда.** (1) «`deposit.do` → bindingId ГАРАНТИРОВАННО» — неверно:
> deposit ни при чём, решает галочка. (2) «UAT-аккаунт не поддерживает привязки» (моя же
> прежняя правка) — тоже неверно: привязки работают (созданы реальные `86782dae`, `870b3f2a`,
> `87029915`). Нельзя лишь **форсировать** их серверно — привилегии `AUTO_PAYMENT` и
> `FORCE_CREATE_BINDING` у UAT-мерчанта отключены (errorCode 13/14). Галочку клиент должен
> поставить сам; предвыбрать её серверно без AUTO_PAYMENT невозможно.
> **В нашем коде бага нет** — `saveAfterPayment` читает `bindingInfo.bindingId` первым;
> при отмеченной галочке сохранится реальная связка и тихий рекуррент пройдёт без формы.
> Если галочки не было → связки нет → срабатывает fallback на форму (см. ниже).
...
### Fallback BNPL-рассрочки на форму оплаты (2026-05-31)

Следствие предыдущего: в UAT **тихий рекуррент невозможен** — реального `bindingId` нет,
`getBindings.do` пуст. Раньше `POST /api/bnpl/{id}/pay` бросал 500
«Не найдена связка карты для списания».

Решение — `BnplService.payInstallmentByClient`: если реальной связки нет, вместо ошибки
регистрируется одностадийный заказ (`register.do`, префикс `INSTF-`, ссылка на контракт
в `AlfaBankOrder`) и возвращается `formUrl`. Клиент платит взнос через форму банка
(ввод карты + 3DS), как первый платёж. Callback `PaymentController` маршрутизирует:
pre-auth → **довзнос-форма (`confirmInstallmentForm`)** → полная оплата.

```
POST /api/bnpl/{id}/pay
  есть реальная связка  → тихое списание (tii=U) → { installments: [...] }
  связки нет (UAT)      → register.do (INSTF-) → { formUrl } → клиент на форму банка
                         → callback → confirmInstallmentForm: orderStatus=2
                         → deposit += взнос, журнал MANUAL, syncInstallmentStatuses
```

- Ответ — обёртка `BnplPayResponse { formUrl | installments }`; фронт `confirmPayNow`:
  `if (res.formUrl) window.location.href = res.formUrl`.
- `confirmInstallmentForm` сохраняет карту только при **реальном** `bindingInfo`
  (`saveFromStatusResponse`) — в UAT синтетический дубликат не плодится.
- `payInstallmentsNow` (тихий, для админа/планировщика) **не тронут** — по-прежнему бросает
  при отсутствии связки. В production карта привяжется на 1-м платеже → форма не нужна.
...
| `bindingId=null` даже после `registerPreAuth`+`deposit`+`clientId` | UAT-аккаунт без включённых привязок (НЕ баг кода) | Реальной связки не получить; в проде merchant с привязками работает |
| 500 «Не найдена связка карты для списания» при рекурренте | Только синтетический `CARDAUTH-`, `getBindings.do` пуст (UAT) | `payInstallmentByClient` → fallback на форму (`formUrl`), см. раздел выше |
...
- [x] **Fallback оплаты взноса на форму** (2026-05-31): если реальной связки нет (UAT) — `payInstallmentByClient` возвращает `formUrl` (одностадийный `INSTF-` заказ) вместо 500; `confirmInstallmentForm` зачитывает оплату в callback; ответ-обёртка `BnplPayResponse`. То же для админа: `payInstallmentByAdmin` / `FullPaymentService.payByDefaultCardOrForm` → `AdminPayResponse{formUrl|order}`
- [x] **Синтетические CARDAUTH-карты больше не сохраняются** (2026-05-31): `saveAfterPayment`/`confirmBinding` сохраняют карту ТОЛЬКО при реальном `bindingInfo.bindingId` (галочка «Сохранить карту» на форме). Нет галочки → карта не сохраняется (раньше плодилась несписываемая `CARDAUTH-...`); удалены `saveFromCardAuthInfo`/`saveFromGetBindings`; существующие синтетические карты вычищены из БД. Корневая причина «карта не привязывается» — отсутствие галочки; форсировать нельзя (`AUTO_PAYMENT`/`FORCE_CREATE_BINDING` → errorCode 13/14)
