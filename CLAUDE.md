...
→ deposit.do → bindingInfo.bindingId (в production)
→ refund.do(100) — возврат 1₽
```

Одностадийный `register.do` возвращает `bindingInfo` только если клиент включил тоггл.
Двухстадийный (`registerPreAuth` + `deposit`) — надёжнее, но **не гарантирует** `bindingId`.

> ⚠️ **ОПРОВЕРГНУТО на UAT (2026-05-31).** Раньше здесь стояло «`deposit.do` → bindingId
> ГАРАНТИРОВАННО». На аккаунте `daniil77_test-api` это **не воспроизводится**: после
> `registerPreAuth` + `deposit` (с `clientId`) `bindingInfo.bindingId == null`
> (лог: `BNPL_FIRST_INSTALLMENT_PAID ... bindingId=null`), а `getBindings.do` отдаёт
> `errorCode=2`. Реальной связки на этом UAT-аккаунте получить нельзя вообще — у него
> не включены привязки/автоплатежи. Карта сохраняется лишь синтетической (`CARDAUTH-...`),
> которую шлюз не знает → тихое списание (`paymentOrderBinding.do`) невозможно.
> Гарантия `bindingId` справедлива только для production-merchant с включёнными привязками.
...
- [x] **Fallback оплаты взноса на форму** (2026-05-31): если реальной связки нет (UAT) — `payInstallmentByClient` возвращает `formUrl` (одностадийный `INSTF-` заказ) вместо 500; `confirmInstallmentForm` зачитывает оплату в callback; ответ-обёртка `BnplPayResponse`
