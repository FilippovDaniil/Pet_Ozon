...
| Метод | URL | Тело | Ответ |
|---|---|---|---|
| POST | `/api/admin/email/send` | `{to, subject, text}` | 200 OK (пусто) |
...
Администратор может отправить произвольное письмо через `POST /api/admin/email/send`.
...
- [x] **Email-уведомления (Spring Mail)** — Яндекс SMTP (порт 465, SMTPS); чек покупателю (`sendOrderReceipt`) при успешной оплате; произвольное письмо от администратора (`POST /api/admin/email/send`); таблица `email_logs` с историей всех отправок; пароль хранится только в `.env` (не попадает в git)
